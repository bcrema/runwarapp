...
class RunController(private val runService: RunService) {
    ...
    fun getDailyStatus(
        ...
        val actionsUsed = runService.getDailyActionCount(principal.user)
        val bandeiraActionsUsed = principal.user.bandeira?.let {
            runService.getBandeiraDailyActionCount(it.id)
        }
        
        return ResponseEntity.ok(DailyStatusResponse(
            userActionsUsed = actionsUsed,
            userActionsRemaining = maxOf(0, 3 - actionsUsed), // TODO: use config
            bandeiraActionsUsed = bandeiraActionsUsed,
            bandeiraActionCap = principal.user.bandeira?.dailyActionCap
        ))
    }
